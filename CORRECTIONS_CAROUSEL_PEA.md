# 🔧 Corrections Appliquées - Carrousel PEA

## 📅 Date
7 février 2026

## 🎯 Objectif
Corriger les bugs de génération identiques à ceux de Carrousel Eco/Bourse et renforcer la qualité des images générées (style "photo de presse").
...
**Problème :** Les images générées écrasaient les anciennes dans Supabase Storage si elles avaient la même position (ex: `imgcaroupea1.png`).
...
- Nettoyage complet des 2 buckets au début de chaque génération :
  - `carousel-pea` (images générées)
  - `carousel-pea-slides` (slides finales)
...
**Problème :** Seul le bucket `carousel-pea-slides` était nettoyé, pas `carousel-pea`.

**Solution :**
- Nettoyage des **2 buckets** au début de `send_to_carousel()`
- Fonctions créées dans `carousel_image_service.py` :
  - `list_image_files()` : liste les fichiers du bucket `carousel-pea`
  - `clear_image_files()` : supprime tous les fichiers du bucket `carousel-pea`
...
Fichiers modifiés :
- `prompts/carousel/pea/generate_image_prompts.py` (style "sunset")
- `prompts/carousel/pea/generate_image_prompts_variant.py` (style "studio")
- `prompts/carousel/pea/generate_image_prompts_manual.py` (avec instructions manuelles)
...
4. ✅ **Nettoyage complet** des 2 buckets (`carousel-pea` + `carousel-pea-slides`)
...
### Services
- ✅ `services/carousel/pea/carousel_image_service.py`
  - Ajout `list_image_files()`
  - Ajout `clear_image_files()`

### Prompts
- ✅ `prompts/carousel/pea/generate_image_prompts.py`
- ✅ `prompts/carousel/pea/generate_image_prompts_variant.py`
- ✅ `prompts/carousel/pea/generate_image_prompts_manual.py`
...
1. Tester une génération complète sur Carrousel PEA
