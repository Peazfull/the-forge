...
class BoursierFranceJob:
    ...
    def _run(self) -> None:
        ...
        buffer_path = f"/tmp/boursier_france_buffer_{datetime.now().strftime('%Y%m%d')}.txt"
        ...
        if not config.use_rss:
            self.errors.append("Mode RSS/DOM requis pour Boursier France")
            self.state = "failed"
            return
        ...
        try:
            ...
            if config.urls_override:
                ...
                if config.rss_use_dom_fallback:
                    articles_dom = fetch_boursier_france_dom_items(
...
_JOB_INSTANCE: Optional[BoursierFranceJob] = None


def get_boursier_france_job() -> BoursierFranceJob:
    global _JOB_INSTANCE
    if _JOB_INSTANCE is None:
        _JOB_INSTANCE = BoursierFranceJob()
