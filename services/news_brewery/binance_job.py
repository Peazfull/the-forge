...
class BinanceJob:
    ...
    def _run(self) -> None:
        ...
        buffer_path = f"/tmp/binance_buffer_{datetime.now().strftime('%Y%m%d')}.txt"
        ...
        try:
            self._log("📰 Mode DOM activé")
            if config.urls_override:
                articles = config.urls_override
            else:
                articles = fetch_binance_dom_items(
...
_JOB_INSTANCE: Optional[BinanceJob] = None


def get_binance_job() -> BinanceJob:
    global _JOB_INSTANCE
    if _JOB_INSTANCE is None:
        _JOB_INSTANCE = BinanceJob()
