...
class BeInCryptoJob:
    ...
    def _run(self) -> None:
        ...
        buffer_path = f"/tmp/beincrypto_buffer_{datetime.now().strftime('%Y%m%d')}.txt"
        ...
        if not config.use_rss:
            self.errors.append("Mode RSS requis pour BeInCrypto")
            self.state = "failed"
            return
        ...
        try:
            ...
            if config.urls_override:
                ...
                if config.rss_use_dom_fallback:
                    articles_dom = fetch_beincrypto_dom_items(
...
_JOB_INSTANCE: Optional[BeInCryptoJob] = None


def get_beincrypto_job() -> BeInCryptoJob:
    global _JOB_INSTANCE
    if _JOB_INSTANCE is None:
        _JOB_INSTANCE = BeInCryptoJob()
