...
class BoursierEconomieJob:
    ...
    def _run(self) -> None:
        ...
        buffer_path = f"/tmp/boursier_economie_buffer_{datetime.now().strftime('%Y%m%d')}.txt"
        ...
        if not config.use_rss:
            self.errors.append("Mode RSS/DOM requis pour Boursier Economie")
            self.state = "failed"
            return
        ...
        try:
            ...
            if config.urls_override:
                ...
                if config.rss_use_dom_fallback:
                    articles_dom = fetch_boursier_dom_items(
...
_JOB_INSTANCE: Optional[BoursierEconomieJob] = None


def get_boursier_economie_job() -> BoursierEconomieJob:
    global _JOB_INSTANCE
    if _JOB_INSTANCE is None:
        _JOB_INSTANCE = BoursierEconomieJob()
