...
class BoursierEtatsUnisJob:
    ...
    def send_to_db(self) -> Dict[str, object]:
        ...
        enriched = enrich_raw_items(
            ...
            flow="news_brewery",
            source_type="news",
            source_name="Boursier Etats-Unis",
    ...
    def _run(self) -> None:
        ...
        buffer_path = f"/tmp/boursier_etats_unis_buffer_{datetime.now().strftime('%Y%m%d')}.txt"
        ...
        if not config.use_rss:
            self.errors.append("Mode RSS/DOM requis pour Boursier Etats-Unis")
            self.state = "failed"
            return
...
_JOB_INSTANCE: Optional[BoursierEtatsUnisJob] = None


def get_boursier_etats_unis_job() -> BoursierEtatsUnisJob:
    global _JOB_INSTANCE
    if _JOB_INSTANCE is None:
        _JOB_INSTANCE = BoursierEtatsUnisJob()
