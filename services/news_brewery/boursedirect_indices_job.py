...
class BourseDirectIndicesJob:
    ...
    def send_to_db(self) -> Dict[str, object]:
        ...
        enriched = enrich_raw_items(
            ...
            flow="news_brewery",
            source_type="news",
            source_name="Bourse Direct Indices",
    ...
    def _run(self) -> None:
        ...
        buffer_path = f"/tmp/boursedirect_indices_buffer_{datetime.now().strftime('%Y%m%d')}.txt"
        ...
        if not config.use_rss:
            self.errors.append("Mode RSS/DOM requis pour Bourse Direct Indices")
            self.state = "failed"
            return
        ...
        try:
            ...
            if config.urls_override:
                ...
                if config.rss_use_dom_fallback:
                    articles_dom = fetch_boursedirect_dom_items(
...
_JOB_INSTANCE: Optional[BourseDirectIndicesJob] = None


def get_boursedirect_indices_job() -> BourseDirectIndicesJob:
    global _JOB_INSTANCE
    if _JOB_INSTANCE is None:
        _JOB_INSTANCE = BourseDirectIndicesJob()
