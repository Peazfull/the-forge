...
class BourseDirectJob:
    ...
    def _run(self) -> None:
        ...
        buffer_path = f"/tmp/boursedirect_buffer_{datetime.now().strftime('%Y%m%d')}.txt"
        ...
        if not config.use_rss:
            self.errors.append("Mode RSS/DOM requis pour Bourse Direct")
            self.state = "failed"
            return
...
_JOB_INSTANCE: Optional[BourseDirectJob] = None


def get_boursedirect_job() -> BourseDirectJob:
    global _JOB_INSTANCE
    if _JOB_INSTANCE is None:
        _JOB_INSTANCE = BourseDirectJob()
