...
from prompts.doss.generate_doss_caption import PROMPT_GENERATE_DOSS_CAPTION
from prompts.doss.generate_doss_linkedin import PROMPT_GENERATE_DOSS_LINKEDIN
from db.supabase_client import get_supabase

REQUEST_TIMEOUT = 30

CAPTION_BUCKET = "carousel-doss-slides"
CAPTION_FILE = "caption.txt"
LINKEDIN_FILE = "linkedin.txt"


def generate_caption_from_doss(title: str, content: str) -> Dict[str, object]:
    ...
    try:
        ...
        response = client.chat.completions.create(
            model="gpt-4o-mini",
            messages=[
                {"role": "system", "content": PROMPT_GENERATE_DOSS_CAPTION},
...
def generate_linkedin_from_doss(title: str, content: str) -> Dict[str, object]:
    if not title or not content:
        return {"status": "error", "message": "Titre ou contenu manquant"}
    ...
    try:
        ...
        response = client.chat.completions.create(
            model="gpt-4o-mini",
            messages=[
                {"role": "system", "content": PROMPT_GENERATE_DOSS_LINKEDIN},
                {"role": "user", "content": user_input},
            ],
            temperature=0.6,
            timeout=REQUEST_TIMEOUT,
