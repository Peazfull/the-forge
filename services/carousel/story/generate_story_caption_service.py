...
from prompts.story.generate_story_caption import PROMPT_GENERATE_STORY_CAPTION
from prompts.story.generate_story_linkedin import PROMPT_GENERATE_STORY_LINKEDIN
from db.supabase_client import get_supabase

REQUEST_TIMEOUT = 30

CAPTION_BUCKET = "carousel-story-slides"
CAPTION_FILE = "caption.txt"
LINKEDIN_FILE = "linkedin.txt"


def generate_caption_from_story(title: str, content: str) -> Dict[str, object]:
    ...
    try:
        ...
        response = client.chat.completions.create(
            model="gpt-4o-mini",
            messages=[
                {"role": "system", "content": PROMPT_GENERATE_STORY_CAPTION},
...
def generate_linkedin_from_story(title: str, content: str) -> Dict[str, object]:
    if not title or not content:
        return {"status": "error", "message": "Titre ou contenu manquant"}
    ...
    try:
        ...
        response = client.chat.completions.create(
            model="gpt-4o-mini",
            messages=[
                {"role": "system", "content": PROMPT_GENERATE_STORY_LINKEDIN},
                {"role": "user", "content": user_input},
            ],
            temperature=0.6,
            timeout=REQUEST_TIMEOUT,
