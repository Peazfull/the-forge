...
from services.carousel.bourse.carousel_bourse_service import (
    insert_items_to_carousel_bourse,
    get_carousel_bourse_items,
    upsert_carousel_bourse_cover,
)
...
from services.carousel.bourse.generate_carousel_texts_service import (
...
class BourseCarouselJob:
    """
    Gère la génération complète d'un carrousel Bourse en arrière-plan (threading).
    """
    ...
    def _log(self, message: str) -> None:
        ...
        # print(f"[BourseCarouselJob] {message}") # Pour debug en console
    ...
    def _run_sequential(self) -> None:
        ...
        try:
            self._log("🚀 Démarrage de la génération Bourse...")

            # Étape 1 : Insertion des items (positions 1 à N)
            self._log("📤 Début insertion en DB...")
            result = insert_items_to_carousel_bourse(self._items_to_process)
            ...
            first_item = items[0]
            cover_result = upsert_carousel_bourse_cover(
                {
    ...
    def _run_optimized(self) -> None:
        ...
        try:
            self._log("🚀⚡ Démarrage génération Bourse (MODE OPTIMISÉ)")
            ...
            from services.carousel.bourse.generate_carousel_texts_service import generate_all_image_prompts_parallel
            ...
            outro_path = os.path.join(
                ...
                "..", "..", "..", "front", "layout", "assets", "carousel", "bourse", "outro_bourse.png"
            )
            if os.path.exists(outro_path):
                from services.carousel.bourse.carousel_slide_service import upload_slide_bytes
...
# Instance globale
_bourse_carousel_job: Optional[BourseCarouselJob] = None


def get_bourse_carousel_job() -> BourseCarouselJob:
    """Retourne l'instance globale du job (avec optimisation activée par défaut)."""
    global _bourse_carousel_job
    if _bourse_carousel_job is None:
        _bourse_carousel_job = BourseCarouselJob(use_optimized=True)
    return _bourse_carousel_job


def reset_bourse_carousel_job() -> None:
    """Réinitialise l'instance globale (pour debug)."""
    global _bourse_carousel_job
    _bourse_carousel_job = None
