...
def clear_carousel_bourse() -> Dict[str, object]:
    """
    Vide la table carousel_bourse (supprime tous les items).
    ...
    try:
        ...
        response = supabase.table("carousel_bourse").delete().neq("id", "00000000-0000-0000-0000-000000000000").execute()
        
        return {
            "status": "success",
            "deleted": len(response.data) if response.data else 0,
            "message": "Carousel Bourse vidé avec succès"
        }
...
def insert_items_to_carousel_bourse(item_ids: List[str]) -> Dict[str, object]:
    """
    Insère les items sélectionnés dans carousel_bourse.
    ...
    try:
        ...
        clear_result = clear_carousel_bourse()
        ...
        # 5. Insérer en batch
        insert_response = supabase.table("carousel_bourse").insert(carousel_items).execute()
        
        return {
            "status": "success",
            "inserted": len(insert_response.data) if insert_response.data else 0,
            "message": f"✅ {len(carousel_items)} items insérés dans Carousel Bourse"
        }
...
def get_carousel_bourse_items() -> Dict[str, object]:
    """
    Récupère tous les items du carousel bourse, triés par position.
...
def upsert_carousel_bourse_cover(source_item: Dict[str, object]) -> Dict[str, object]:
    """
    Crée ou met à jour la ligne cover (position = 0) dans carousel_bourse.
    """
    try:
        supabase = get_supabase()
        
        existing = supabase.table("carousel_bourse").select("id").eq("position", 0).limit(1).execute()
        payload = {
        ...
        if existing.data:
            ...
            supabase.table("carousel_bourse").update(payload).eq("id", cover_id).execute()
        ...
        inserted = supabase.table("carousel_bourse").insert(insert_payload).execute()
