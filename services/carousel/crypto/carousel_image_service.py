"""
Service de génération et sauvegarde d'images pour carousel_crypto
...
from prompts.carousel.crypto.generate_image_prompts_manual import PROMPT_GENERATE_IMAGE_PROMPT_MANUAL
import json
...
STORAGE_BUCKET = "carousel-crypto"
...
def get_image_path(position: int) -> str:
    ...
    Returns:
        Chemin complet vers imgcaroucrypto{position}.png
    """
    os.makedirs(CAROUSEL_IMAGE_DIR, exist_ok=True)
    return os.path.join(CAROUSEL_IMAGE_DIR, f"imgcaroucrypto{position}.png")
...
def upload_image_bytes(image_bytes: bytes, position: int) -> Dict[str, object]:
    ...
    try:
        supabase = get_supabase()
        object_path = f"imgcaroucrypto{position}.png"
...
def save_image_base64(image_base64: str, position: int) -> Dict[str, object]:
    ...
    try:
        ...
        return {
            ...
            "message": f"Image sauvegardée : imgcaroucrypto{position}.png"
        }
...
def save_image_to_crypto(item_id: str, image_url: str) -> Dict[str, object]:
    """Sauvegarde l'URL de l'image dans la table carousel_crypto."""
    try:
        supabase = get_supabase()
...
def save_prompt_image_3_to_db(item_id: str, prompt_image_3: str) -> Dict[str, object]:
    ...
    Args:
        item_id: UUID de l'item dans carousel_crypto
    ...
    try:
        ...
        response = supabase.table("carousel_crypto").update({
