...
from prompts.carousel.crypto.generate_carousel_texts import PROMPT_GENERATE_CAROUSEL_TEXTS
from prompts.carousel.crypto.generate_image_prompts import PROMPT_GENERATE_IMAGE_PROMPT
from prompts.carousel.crypto.generate_image_prompts_variant import PROMPT_GENERATE_IMAGE_PROMPT_VARIANT
...
def generate_all_carousel_texts() -> Dict[str, object]:
    """
    Génère les textes carousel pour tous les items dans carousel_crypto.
    ...
    try:
        ...
        # Récupérer tous les items de carousel_crypto (triés par position)
        ...
        if not items:
            return {
                "status": "error",
                "message": "Aucun item dans carousel_crypto",
                "total": 0,
                "success": 0,
                "errors": 0,
                "details": []
        ...
        for item in items:
            ...
            if all_success:
                # Mise à jour en DB
                update_response = supabase.table("carousel_crypto").update({
