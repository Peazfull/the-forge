...
from prompts.stories.generate_stories_texts import PROMPT_GENERATE_STORIES_TEXTS
from prompts.stories.rewrite_stories_article import PROMPT_REWRITE_STORIES_ARTICLE
from prompts.stories.generate_stories_image_prompts import PROMPT_GENERATE_STORIES_IMAGE_PROMPT

REQUEST_TIMEOUT = 30
MAX_WORKERS_IMAGES = 6  # Paralléliser jusqu'à 6 images en même temps


def generate_stories_texts(raw_text: str) -> dict:
    ...
    rewrite = client.chat.completions.create(
        model="gpt-4o-mini",
        messages=[
            {"role": "system", "content": PROMPT_REWRITE_STORIES_ARTICLE},
    ...
    response = client.chat.completions.create(
        model="gpt-4o-mini",
        messages=[
            {"role": "system", "content": PROMPT_GENERATE_STORIES_TEXTS},
...
def generate_stories_image_prompt(title: str, content: str) -> dict:
    user_input = f"TITRE: {title}\n\nCONTENU: {content}"
    client = OpenAI(api_key=st.secrets["OPENAI_API_KEY"])
    response = client.chat.completions.create(
        model="gpt-4o-mini",
        messages=[
            {"role": "system", "content": PROMPT_GENERATE_STORIES_IMAGE_PROMPT},
...
def generate_stories_image(prompt: str) -> dict:
...
def generate_stories_images_parallel(
    state: Dict,
    slide_data: list,
    ...
    """
    Génère les 5 images de Stories en parallèle.
        ...
        try:
            ...
            if not prompt:
                if idx == 0:
                    ...
                    p = generate_stories_image_prompt(
