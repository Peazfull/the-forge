...
from prompts.stories.generate_stories_caption import PROMPT_GENERATE_STORIES_CAPTION
from prompts.stories.generate_stories_linkedin import PROMPT_GENERATE_STORIES_LINKEDIN
from db.supabase_client import get_supabase

REQUEST_TIMEOUT = 30

CAPTION_BUCKET = "carousel-stories-slides"
CAPTION_FILE = "caption.txt"
LINKEDIN_FILE = "linkedin.txt"


def generate_caption_from_stories(title: str, content: str) -> Dict[str, object]:
    ...
    try:
        ...
        response = client.chat.completions.create(
            model="gpt-4o-mini",
            messages=[
                {"role": "system", "content": PROMPT_GENERATE_STORIES_CAPTION},
...
def generate_linkedin_from_stories(title: str, content: str) -> Dict[str, object]:
    if not title or not content:
        return {"status": "error", "message": "Titre ou contenu manquant"}

    user_input = f"STORIES:\nTITRE: {title}\n\nCONTENU: {content}"
    try:
        client = OpenAI(api_key=st.secrets["OPENAI_API_KEY"])
        response = client.chat.completions.create(
            model="gpt-4o-mini",
            messages=[
                {"role": "system", "content": PROMPT_GENERATE_STORIES_LINKEDIN},
                {"role": "user", "content": user_input},
            ],
            temperature=0.6,
            timeout=REQUEST_TIMEOUT,
