...
def _format_french_date(dt: datetime | None = None) -> str:
    if dt is None:
        ...
        if weekly_date:
            try:
                dt = datetime.fromisoformat(weekly_date)
            except Exception:
                dt = datetime.now()
        else:
            dt = datetime.now()
...
def _get_flop10_weekly_eu() -> list[dict]:
    try:
        data = get_top_flop_weekly("EU", limit=10)
        if data.get("status") == "success":
            return data.get("flop", []) or []
...
def _get_top10_weekly_fr() -> list[dict]:
    try:
...
def _get_commodities_weekly() -> list[dict]:
    try:
        data = get_weekly_performances(COMMODITIES_MAJOR)
        if data.get("status") == "success":
            return data.get("items", []) or []
        return []
    except Exception:
...
def _get_fx_weekly_eu() -> list[dict]:
    try:
        data = get_weekly_performances(EU_FX_PAIRS)
        if data.get("status") == "success":
...
def _get_crypto_weekly() -> list[dict]:
    try:
        data = get_weekly_performances(CRYPTO_MAJOR)
        if data.get("status") == "success":
            items = data.get("items", []) or []
...
def refresh_weekly_slides() -> None:
    """Force un refresh des données et un rerun pour la preview/export."""
    st.cache_data.clear()
    st.session_state.weekly_refresh_token = str(time.time())


st.title("TheArtist - Weekly")
st.divider()
...
if st.button("📦 Préparer export Weekly", use_container_width=True):
    export_data = build_weekly_exports(slide_files)
    st.session_state.weekly_export_zip = export_data["zip"]
    st.session_state.weekly_export_pdf = export_data["pdf"]
    st.session_state.weekly_export_count = export_data["count"]

if st.session_state.get("weekly_export_zip"):
    if st.session_state.get("weekly_export_count", 0) == 0:
        st.warning("Aucune slide disponible pour l'export.")
    else:
        st.caption(f"{st.session_state.get('weekly_export_count', 0)} slides prêtes")
        st.download_button(
            "⬇️ Télécharger PNG (ZIP)",
            data=st.session_state.weekly_export_zip,
            file_name="weekly_slides.zip",
            mime="application/zip",
            use_container_width=True,
        )
        st.download_button(
            "⬇️ Télécharger PDF",
            data=st.session_state.weekly_export_pdf,
            file_name="weekly_slides.pdf",
...
if st.button("✉️ Envoyer par email", use_container_width=True):
    try:
        ...
        if export_data.get("count", 0) == 0:
            ...
            subject = f"Weekly - {date_str}"
            caption_text = _load_fixed_caption() or "Caption non disponible."
            attachments = [
                ("weekly_slides.zip", export_data["zip"], "application/zip"),
                ("weekly_slides.pdf", export_data["pdf"], "application/pdf"),
            ]
...
with st.expander("📝 Caption Instagram", expanded=False):
    ...
    if not caption_text:
        ...
        st.warning("Caption fixe manquante : utilise le fichier prompts/weekly/fixed_caption.txt")
