...
📈 MARKET CLOSE — EUROPE
===========================================
Close du jour (close vs close précédent)
"""

import streamlit as st
import pandas as pd

from services.marketbrewery.market_close_service import (
    refresh_data,
    get_close_top_flop,
...
@st.cache_data
def get_close_top_flop_cached(symbols, limit=10):
    """
    Version cachée de get_close_top_flop
    """
    return get_close_top_flop(symbols, limit)
...
def render_zone_section(symbols, zone_name, zone_flag):
    ...
    """
    close_data = get_close_top_flop_cached(symbols, limit=10)
    num_assets = len(close_data.get("top", [])) if close_data.get("status") == "success" else 0
    ...
    with col_top:
        st.markdown('<p class="section-title">🟢 Top 10 Close</p>', unsafe_allow_html=True)
        if close_data["status"] == "success" and close_data["top"]:
            df_top = create_performance_table(close_data["top"])
    ...
    with col_flop:
        st.markdown('<p class="section-title">🔴 Flop 10 Close</p>', unsafe_allow_html=True)
        if close_data["status"] == "success" and close_data["flop"]:
            df_flop = create_performance_table(close_data["flop"])
...
def render_simple_section(symbols, zone_name, zone_flag):
    ...
    """
    data = get_close_performances(symbols)
...
<div class="market-header">
    <h1>📈 Market Close</h1>
    <p>Europe Close Tracker · Close vs Close précédent</p>
</div>
""", unsafe_allow_html=True)
...
with col_date:
    last_date = get_last_close_date()
    if last_date:
        st.markdown(f'<div class="date-badge">📅 Dernière date en base : {last_date}</div>', unsafe_allow_html=True)
    else:
...
<div style="text-align: center; color: #6b7280; font-size: 14px; padding: 24px 0;">
    ...
    <p style="margin: 8px 0 0 0; font-size: 12px;">Market Close v1.0</p>
</div>
""", unsafe_allow_html=True)
