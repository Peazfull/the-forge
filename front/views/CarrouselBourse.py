...
from services.carousel.bourse.carousel_bourse_service import insert_items_to_carousel_bourse, get_carousel_bourse_items, upsert_carousel_bourse_cover
from services.carousel.bourse.generate_carousel_texts_service import generate_all_carousel_texts, update_carousel_text
from services.carousel.bourse.generate_carousel_caption_service import (
...
from services.carousel.bourse.carousel_image_service import (
    generate_and_save_carousel_image,
    generate_prompt_image_3,
    save_prompt_image_3_to_db,
    read_carousel_image,
    save_image_base64,
    save_image_to_bourse
)
from services.carousel.bourse.carousel_slide_service import (
...
# Toujours initialiser bourse_selected_items si absent (indépendant de bourse_initialized)
if "bourse_selected_items" not in st.session_state:
    st.session_state.bourse_selected_items = []

if "bourse_modal_item" not in st.session_state:
    st.session_state.bourse_modal_item = None

if "bourse_preview_mode" not in st.session_state:
...
def open_modal(item):
    ...
    st.session_state.bourse_modal_item = item
...
def generate_all_slide_previews():
    ...
    outro_path = os.path.join(
        ...
        "..", "layout", "assets", "carousel", "bourse", "outro.png"
    )
...
def send_to_carousel():
    ...
    from services.carousel.bourse.bourse_carousel_job import get_bourse_carousel_job
    
    job = get_bourse_carousel_job()
    if job.state == "running":
        st.warning("⚠️ Une génération est déjà en cours")
        return
    
    # Nettoyer les caches avant de lancer une nouvelle génération
    st.session_state.bourse_selected_items = st.session_state.get("bourse_selected_items", [])
    st.session_state.carousel_images = {}
    st.session_state.carousel_image_models = {}
    st.session_state.slide_previews = {}
    
    # Lancer le job en threading
    job.start(st.session_state.bourse_selected_items)
    st.rerun()
...
# Remplacées par l'architecture threading (bourse_carousel_job.py)


def toggle_preview_mode():
    """Bascule entre tri par score et tri par position"""
    st.session_state.bourse_preview_mode = not st.session_state.bourse_preview_mode
...
def get_item_position(item_id):
    ...
    if item_id in st.session_state.bourse_selected_items:
        return st.session_state.bourse_selected_items.index(item_id) + 1
    return None
...
def set_item_position(item_id, target_position):
    ...
    if item_id not in st.session_state.bourse_selected_items:
        return
    
    current_idx = st.session_state.bourse_selected_items.index(item_id)
    ...
    if 0 <= target_idx < len(st.session_state.bourse_selected_items):
        other_item_id = st.session_state.bourse_selected_items[target_idx]
        st.session_state.bourse_selected_items[target_idx] = item_id
        st.session_state.bourse_selected_items[current_idx] = other_item_id
        return
    
    # Sinon, on réinsère (fallback)
    item = st.session_state.bourse_selected_items.pop(current_idx)
    st.session_state.bourse_selected_items.insert(target_idx, item)
...
def move_item_up(item_id):
    ...
    if item_id not in st.session_state.bourse_selected_items:
        return
    idx = st.session_state.bourse_selected_items.index(item_id)
    if idx == 0:
        return
    st.session_state.bourse_selected_items[idx], st.session_state.bourse_selected_items[idx - 1] = (
        st.session_state.bourse_selected_items[idx - 1],
        st.session_state.bourse_selected_items[idx],
    )
...
def move_item_down(item_id):
    ...
    if item_id not in st.session_state.bourse_selected_items:
        return
    idx = st.session_state.bourse_selected_items.index(item_id)
    if idx >= len(st.session_state.bourse_selected_items) - 1:
        return
    st.session_state.bourse_selected_items[idx], st.session_state.bourse_selected_items[idx + 1] = (
        st.session_state.bourse_selected_items[idx + 1],
        st.session_state.bourse_selected_items[idx],
...
with st.expander("📰 Bulletin Bourse", expanded=False):
    ...
    with col_date:
        filter_date = st.date_input("Du (date)", value=default_dt.date(), key="bourse_filter_date")
    with col_time:
        filter_time = st.time_input(
            "Du (heure)",
            value=default_dt.time().replace(second=0, microsecond=0),
            key="bourse_filter_time"
        )
    with col_apply:
        if st.button("Appliquer", key="apply_bourse_filter", use_container_width=True):
            st.session_state.bourse_filter_start = datetime.combine(filter_date, filter_time)
            st.session_state.bourse_initialized = False
            st.rerun()
    with col_reset:
        if st.button("Reset", key="reset_bourse_filter", use_container_width=True, type="secondary"):
            st.session_state.bourse_filter_start = None
            st.session_state.bourse_initialized = False
            st.rerun()
    ...
    if not items:
        ...
        if "bourse_selected_items" in st.session_state:
            st.session_state.bourse_selected_items = [
                item_id for item_id in st.session_state.bourse_selected_items
                if item_id in allowed_ids
            ]
        
        # Header
        selected_count = len(st.session_state.bourse_selected_items)
        ...
        with col_preview_btn:
            if selected_count > 0:
                if st.session_state.bourse_preview_mode:
        ...
        if selected_count > 0:
            ...
            for idx, item_id in enumerate(st.session_state.bourse_selected_items, start=1):
        ...
        if st.session_state.bourse_preview_mode and selected_count > 0:
            # Mode preview : afficher dans l'ordre de bourse_selected_items
            # Créer un dict pour accès rapide aux items par ID
            items_dict = {item["id"]: item for item in items}
            
            # Reconstruire la liste dans l'ordre de bourse_selected_items
            display_items = []
            for item_id in st.session_state.bourse_selected_items:
                if item_id in items_dict:
                    display_items.append(items_dict[item_id])
            
            # Ajouter les items non-sélectionnés à la fin
            unselected_items = [item for item in items if item["id"] not in st.session_state.bourse_selected_items]
        ...
        for idx, item in enumerate(display_items, start=1):
            ...
            with col_view:
                if st.button("👁️", key=f"view_bourse_{item_id}", help="Voir le détail"):
                    open_modal(item)
...
with st.expander("🎨 Textes Carousel", expanded=False):
    ...
    if carousel_data["status"] == "error":
        ...
        st.info("Aucun item · Envoyez d'abord des items depuis 'Bulletin Bourse'")
    else:
            ...
            with col_controls:
                ...
                if uploaded_file is not None:
                    ...
                    if file_id != last_file_id:
                        ...
                        try:
                            from services.carousel.bourse.carousel_image_service import get_image_path
                        ...
                        try:
                            from services.carousel.bourse.carousel_image_service import upload_image_bytes
        ...
        if st.session_state.get("carousel_export_zip"):
            if st.session_state.get("carousel_export_count", 0) == 0:
                ...
                st.download_button(
                    ...
                    file_name="carousel_bourse_slides.zip",
                ...
                st.download_button(
                    ...
                    file_name="carousel_bourse_slides.pdf",
...
if st.button("✉️ Envoyer par email", use_container_width=True):
    try:
        ...
        if export_data.get("count", 0) == 0:
            ...
            subject = f"Carrousel Bourse - {date_str}"
            caption_text = st.session_state.caption_text_area.strip() or read_caption_text()
            body = caption_text or "Caption non disponible."
            attachments = [
                ("carousel_bourse_slides.zip", export_data["zip"], "application/zip"),
                ("carousel_bourse_slides.pdf", export_data["pdf"], "application/pdf")
            ]
...
if st.session_state.bourse_modal_item:
    item = st.session_state.bourse_modal_item
