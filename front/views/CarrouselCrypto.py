...
from services.carousel.crypto.carousel_crypto_service import insert_items_to_carousel_crypto, get_carousel_crypto_items, upsert_carousel_crypto_cover
from services.carousel.crypto.generate_carousel_texts_service import generate_all_carousel_texts, update_carousel_text
...
# ======================================================
if st.session_state.get("active_carousel") != "crypto":
    st.session_state.active_carousel = "crypto"
...
if "crypto_initialized" not in st.session_state:
    st.session_state.crypto_initialized = False

# Toujours initialiser crypto_selected_items si absent (indépendant de crypto_initialized)
if "crypto_selected_items" not in st.session_state:
    st.session_state.crypto_selected_items = []

if "crypto_modal_item" not in st.session_state:
    st.session_state.crypto_modal_item = None

if "crypto_preview_mode" not in st.session_state:
    st.session_state.crypto_preview_mode = False

if "crypto_filter_start" not in st.session_state:
    st.session_state.crypto_filter_start = None

if "crypto_auto_caption_done" not in st.session_state:
    st.session_state.crypto_auto_caption_done = False

# ======================================================
# FONCTIONS
# ======================================================

def fetch_top_crypto_items(start_dt: datetime | None = None):
    """Récupère le top CRYPTO."""
    try:
        supabase = get_supabase()

        query = supabase.table("brew_items").select(
            "id, title, content, tags, labels, score_global"
        ).eq("tags", "CRYPTO").not_.is_("score_global", "null")
        if start_dt:
...
def toggle_selection(item_id):
    ...
    if item_id in st.session_state.crypto_selected_items:
        st.session_state.crypto_selected_items.remove(item_id)
    else:
        if len(st.session_state.crypto_selected_items) < 10:
            st.session_state.crypto_selected_items.append(item_id)


def open_modal(item):
    """Ouvre le modal avec le détail de l'item"""
    st.session_state.crypto_modal_item = item
...
def generate_all_slide_previews():
    ...
    for item in items_sorted:
        ...
        if not image_url:
            try:
                supabase = get_supabase()
                image_bytes = supabase.storage.from_("carousel-crypto").download(f"image_{item_id}.png")
            except Exception:
    ...
    outro_path = os.path.join(
        ...
        "..", "layout", "assets", "carousel", "crypto", "outro_crypto.png"
    )
...
def send_to_carousel():
    ...
    from services.carousel.crypto.crypto_carousel_job import get_crypto_carousel_job
    
    job = get_crypto_carousel_job()
    if job.state == "running":
        st.warning("⚠️ Une génération est déjà en cours")
        return
    
    # Nettoyer les caches avant de lancer une nouvelle génération
    st.session_state.crypto_selected_items = st.session_state.get("crypto_selected_items", [])
    st.session_state.carousel_images = {}
    st.session_state.carousel_image_models = {}
    st.session_state.slide_previews = {}
    
    # Lancer le job en threading
    job.start(st.session_state.crypto_selected_items)
    st.rerun()


# ANCIENNES FONCTIONS SUPPRIMÉES (_finalize_generation et process_generation_queue)
# Remplacées par l'architecture threading (crypto_carousel_job.py)


def toggle_preview_mode():
    """Bascule entre tri par score et tri par position"""
    st.session_state.crypto_preview_mode = not st.session_state.crypto_preview_mode
...
def get_item_position(item_id):
    ...
    if item_id in st.session_state.crypto_selected_items:
        return st.session_state.crypto_selected_items.index(item_id) + 1
    return None
...
def set_item_position(item_id, target_position):
    ...
    if item_id not in st.session_state.crypto_selected_items:
        return
    
    current_idx = st.session_state.crypto_selected_items.index(item_id)
    ...
    if 0 <= target_idx < len(st.session_state.crypto_selected_items):
        other_item_id = st.session_state.crypto_selected_items[target_idx]
        st.session_state.crypto_selected_items[target_idx] = item_id
        st.session_state.crypto_selected_items[current_idx] = other_item_id
        return
    
    # Sinon, on réinsère (fallback)
    item = st.session_state.crypto_selected_items.pop(current_idx)
    st.session_state.crypto_selected_items.insert(target_idx, item)
...
def move_item_up(item_id):
    ...
    if item_id not in st.session_state.crypto_selected_items:
        return
    idx = st.session_state.crypto_selected_items.index(item_id)
    if idx == 0:
        return
    st.session_state.crypto_selected_items[idx], st.session_state.crypto_selected_items[idx - 1] = (
        st.session_state.crypto_selected_items[idx - 1],
        st.session_state.crypto_selected_items[idx],
    )
...
def move_item_down(item_id):
    ...
    if item_id not in st.session_state.crypto_selected_items:
        return
    idx = st.session_state.crypto_selected_items.index(item_id)
    if idx >= len(st.session_state.crypto_selected_items) - 1:
        return
    st.session_state.crypto_selected_items[idx], st.session_state.crypto_selected_items[idx + 1] = (
        st.session_state.crypto_selected_items[idx + 1],
        st.session_state.crypto_selected_items[idx],
...
# Traitement séquentiel REMPLACÉ PAR THREADING
# ======================================================
# THREADED GENERATION STATUS
# ======================================================
from services.carousel.crypto.crypto_carousel_job import get_crypto_carousel_job
...
with st.expander("📰 Bulletin Crypto", expanded=False):
    ...
    with col_date:
        filter_date = st.date_input("Du (date)", value=default_dt.date(), key="crypto_filter_date")
    with col_time:
        filter_time = st.time_input(
            "Du (heure)",
            value=default_dt.time().replace(second=0, microsecond=0),
            key="crypto_filter_time"
        )
    with col_apply:
        if st.button("Appliquer", key="apply_crypto_filter", use_container_width=True):
            st.session_state.crypto_filter_start = datetime.combine(filter_date, filter_time)
            st.session_state.crypto_initialized = False
            st.rerun()
    with col_reset:
        if st.button("Reset", key="reset_crypto_filter", use_container_width=True, type="secondary"):
            st.session_state.crypto_filter_start = None
            st.session_state.crypto_initialized = False
            st.rerun()
    
    if st.session_state.crypto_filter_start:
        st.caption(f"Filtre actif : depuis {st.session_state.crypto_filter_start.strftime('%d/%m/%Y %H:%M')}")
    else:
        st.caption("Filtre inactif : top 25 CRYPTO (toutes dates)")
    
    st.markdown("---")
    
    # Fetch data
    items = fetch_top_crypto_items(start_dt=st.session_state.crypto_filter_start)
    ...
    if not items:
        ...
        if "crypto_selected_items" in st.session_state:
            st.session_state.crypto_selected_items = [
                item_id for item_id in st.session_state.crypto_selected_items
                if item_id in allowed_ids
            ]
        
        # Header
        selected_count = len(st.session_state.crypto_selected_items)
        ...
        with col_preview_btn:
            if selected_count > 0:
                if st.session_state.crypto_preview_mode:
        ...
        if selected_count > 0:
            ...
            for idx, item_id in enumerate(st.session_state.crypto_selected_items, start=1):
        ...
        if st.session_state.crypto_preview_mode and selected_count > 0:
            # Mode preview : afficher dans l'ordre de crypto_selected_items
            # Créer un dict pour accès rapide aux items par ID
            items_dict = {item["id"]: item for item in items}
            
            # Reconstruire la liste dans l'ordre de crypto_selected_items
            display_items = []
            for item_id in st.session_state.crypto_selected_items:
                if item_id in items_dict:
                    display_items.append(items_dict[item_id])
            
            # Ajouter les items non-sélectionnés à la fin
            unselected_items = [item for item in items if item["id"] not in st.session_state.crypto_selected_items]
        ...
        for idx, item in enumerate(display_items, start=1):
            ...
            with col_check:
                # Checkbox logic
                is_selected = item_id in st.session_state.crypto_selected_items
                is_disabled = (not is_selected) and (selected_count >= 10)
                
                st.checkbox(
                    label="",
                    value=is_selected,
                    key=f"check_crypto_{item_id}",
            ...
            with col_view:
                if st.button("👁️", key=f"view_crypto_{item_id}", help="Voir le détail"):
                    open_modal(item)
...
with st.expander("🎨 Textes Carousel", expanded=False):
    ...
    if carousel_data["status"] == "error":
        ...
        st.info("Aucun item · Envoyez d'abord des items depuis 'Bulletin Crypto'")
    else:
            ...
            with col_image:
                ...
                if existing_image_url and not get_model_from_image_url(existing_image_url) and model_name:
                    ...
                    if model_tag:
                        ...
                        save_image_to_crypto(item_id, image_url)
            ...
            with col_controls:
                ...
                if uploaded_file is not None:
                    ...
                    if file_id != last_file_id:
                        ...
                        try:
                            from services.carousel.crypto.carousel_image_service import get_image_path
                        ...
                        try:
                            from services.carousel.crypto.carousel_image_service import upload_image_bytes
            ...
            with col:
                ...
                if item_id == "outro":
                    ...
                    if "slide_outro.png" in stored_files:
                        ...
                        st.warning("outro_crypto.png introuvable")
                    continue
        ...
        if st.session_state.get("carousel_export_zip"):
            if st.session_state.get("carousel_export_count", 0) == 0:
                ...
                st.download_button(
                    ...
                    file_name="carousel_crypto_slides.zip",
                ...
                st.download_button(
                    ...
                    file_name="carousel_crypto_slides.pdf",
...
with st.expander("📝 Caption Instagram", expanded=False):
    ...
    if carousel_data["status"] == "error":
        ...
        if not st.session_state.caption_text_area and not st.session_state.crypto_auto_caption_done:
            ...
            st.session_state.crypto_auto_caption_done = True
...
if st.button("✉️ Envoyer par email", use_container_width=True):
    try:
        ...
        if export_data.get("count", 0) == 0:
            ...
            subject = f"Carrousel Crypto - {date_str}"
            caption_text = st.session_state.caption_text_area.strip() or read_caption_text()
            body = caption_text or "Caption non disponible."
            attachments = [
                ("carousel_crypto_slides.zip", export_data["zip"], "application/zip"),
                ("carousel_crypto_slides.pdf", export_data["pdf"], "application/pdf")
            ]
...
if st.session_state.crypto_modal_item:
    item = st.session_state.crypto_modal_item
