...
from services.news_brewery.boursier_etats_unis_job import (
    JobConfig as BoursierEtatsUnisJobConfig,
    get_boursier_etats_unis_job,
)
from services.news_brewery.rss_utils import (
    ...
    fetch_boursier_etats_unis_dom_items,
    fetch_dom_items,
    fetch_rss_items,
    merge_article_items,
)
...
with st.expander("▸ Job — Boursier Etats-Unis", expanded=False):
    boursier_etats_unis_job = get_boursier_etats_unis_job()
    col_open, col_launch, col_clear = st.columns([2, 1, 1])

    with col_open:
        st.link_button("🔗 Ouvrir l’URL", "https://www.boursier.com/actualites/etats-unis/")
    with col_launch:
        boursier_etats_unis_launch = st.button("▶️ Lancer", use_container_width=True, key="boursier_etats_unis_launch")
    with col_clear:
        boursier_etats_unis_clear = st.button("🧹 Clear", use_container_width=True, key="boursier_etats_unis_clear")

    with st.expander("Fenêtre temporelle", expanded=True):
        boursier_etats_unis_mode = st.radio(
            "Mode",
            options=["Aujourd’hui", "Dernières X heures"],
            horizontal=True,
            index=1,
            key="boursier_etats_unis_mode",
        )
        boursier_etats_unis_hours_window = st.slider(
            "Dernières X heures",
            min_value=1,
            max_value=24,
            value=6,
            step=1,
            key="boursier_etats_unis_hours_window",
        )
    ...
    with st.expander("Settings", expanded=False):
        ...
        col_max_total, col_max_per = st.columns(2)
        with col_max_total:
            boursier_etats_unis_max_articles_total = st.number_input(
                "Max articles total",
                min_value=1,
                max_value=100,
                value=20,
                step=1,
                key="boursier_etats_unis_max_total",
            )
        with col_max_per:
            boursier_etats_unis_max_articles_per = st.number_input(
                "Max articles par bulletin",
                min_value=1,
                max_value=20,
                value=20,
                step=1,
                key="boursier_etats_unis_max_per",
            )

        st.markdown("**Timing**")
        col_wait_min, col_wait_max = st.columns(2)
        with col_wait_min:
            boursier_etats_unis_wait_min_action = st.number_input(
                "Wait min action (s)",
                min_value=0.1,
                max_value=5.0,
                value=0.6,
                step=0.1,
                key="boursier_etats_unis_wait_min",
            )
        with col_wait_max:
            boursier_etats_unis_wait_max_action = st.number_input(
                "Wait max action (s)",
                min_value=0.2,
                max_value=8.0,
                value=2.5,
                step=0.1,
                key="boursier_etats_unis_wait_max",
            )

        boursier_etats_unis_shuffle_urls = st.checkbox(
            "Shuffle URLs",
            value=True,
            key="boursier_etats_unis_shuffle",
        )
        boursier_etats_unis_dry_run = st.checkbox(
            "DRY RUN",
            value=False,
            key="boursier_etats_unis_dry_run",
        )

        st.markdown("**Safety**")
        col_err, col_timeout = st.columns(2)
        with col_err:
            boursier_etats_unis_max_consecutive_errors = st.number_input(
                "Max erreurs consécutives",
                min_value=1,
                max_value=10,
                value=3,
                step=1,
                key="boursier_etats_unis_max_errors",
            )
        with col_timeout:
            boursier_etats_unis_global_timeout_minutes = st.number_input(
                "Timeout global job (min)",
                min_value=1,
                max_value=60,
                value=15,
                step=1,
                key="boursier_etats_unis_timeout",
            )

        boursier_etats_unis_remove_buffer = st.checkbox(
            "Supprimer buffer après succès",
            value=True,
            key="boursier_etats_unis_remove_buffer",
        )

        st.markdown("**Source URLs**")
        boursier_etats_unis_rss_feed_url = st.text_input(
            "RSS feed",
            value="https://www.boursier.com/actualites/etats-unis",
            key="boursier_etats_unis_rss_feed",
        )
        boursier_etats_unis_use_rss = st.checkbox(
            "Mode RSS/DOM",
            value=True,
            key="boursier_etats_unis_use_rss",
        )
        boursier_etats_unis_use_firecrawl = st.checkbox(
            "Scraper articles via Firecrawl",
            value=True,
            key="boursier_etats_unis_use_firecrawl",
        )
        boursier_etats_unis_rss_ignore_time_filter = st.checkbox(
            "Ignorer filtre temporel RSS",
            value=False,
            key="boursier_etats_unis_rss_ignore_time",
        )
        boursier_etats_unis_rss_use_dom_fallback = st.checkbox(
            "Compléter via DOM (Etats-Unis)",
            value=True,
    ...
    if boursier_etats_unis_use_rss:
        col_clear, col_uncheck = st.columns(2)
        with col_clear:
            if st.button("🧹 Clear liste", use_container_width=True, key="boursier_etats_unis_rss_clear"):
                st.session_state.boursier_etats_unis_rss_candidates = []
                st.rerun()
        with col_uncheck:
            if st.button("☐ Décocher tout", use_container_width=True, key="boursier_etats_unis_rss_uncheck_all"):
                for idx in range(len(st.session_state.boursier_etats_unis_rss_candidates)):
                    st.session_state[f"boursier_etats_unis_rss_pick_{idx}"] = False
                st.rerun()

        if st.session_state.boursier_etats_unis_rss_candidates:
            st.caption("Sélectionne les articles à traiter :")
            for idx, item in enumerate(st.session_state.boursier_etats_unis_rss_candidates):
                label = f"{item.get('title','')}".strip() or item.get("url", "")
                key = f"boursier_etats_unis_rss_pick_{idx}"
                if key not in st.session_state:
                    st.session_state[key] = True
                checked = st.checkbox(label, key=key)
                if checked:
                    boursier_etats_unis_selected_urls.append(item)
            st.caption(f"{len(boursier_etats_unis_selected_urls)} article(s) sélectionné(s)")
        else:
            st.caption("Clique sur Lancer pour charger la liste.")

    if st.session_state.boursier_etats_unis_rss_candidates:
        if st.button("🧭 Scrapper les articles", use_container_width=True, key="boursier_etats_unis_scrape_articles"):
            if not boursier_etats_unis_selected_urls:
                st.error("Sélectionne au moins un article.")
            else:
                ...
                st.session_state.boursier_etats_unis_show_json_state = False
                st.session_state.boursier_etats_unis_json_ready = False
                config = BoursierEtatsUnisJobConfig(
                    entry_url="https://www.boursier.com/actualites/etats-unis",
                    mode="today" if boursier_etats_unis_mode == "Aujourd’hui" else "last_hours",
                    hours_window=int(boursier_etats_unis_hours_window),
                    max_articles_total=int(boursier_etats_unis_max_articles_total),
                    max_articles_per_bulletin=int(boursier_etats_unis_max_articles_per),
                    wait_min_action=float(boursier_etats_unis_wait_min_action),
                    wait_max_action=float(boursier_etats_unis_wait_max_action),
                    shuffle_urls=bool(boursier_etats_unis_shuffle_urls),
                    dry_run=bool(boursier_etats_unis_dry_run),
                    max_consecutive_errors=int(boursier_etats_unis_max_consecutive_errors),
                    global_timeout_minutes=int(boursier_etats_unis_global_timeout_minutes),
                    remove_buffer_after_success=bool(boursier_etats_unis_remove_buffer),
                    use_rss=bool(boursier_etats_unis_use_rss),
                    rss_feed_url=boursier_etats_unis_rss_feed_url,
                    rss_ignore_time_filter=bool(boursier_etats_unis_rss_ignore_time_filter),
                    rss_use_dom_fallback=bool(boursier_etats_unis_rss_use_dom_fallback),
                    use_firecrawl=bool(boursier_etats_unis_use_firecrawl),
                    urls_override=boursier_etats_unis_selected_urls,
                )
                boursier_etats_unis_job.start(config)
                st.success("Scraping lancé.")
    ...
    if boursier_etats_unis_launch:
        if boursier_etats_unis_use_rss:
            ...
            st.session_state.boursier_etats_unis_json_ready = False
            rss_items = fetch_rss_items(
                feed_url=boursier_etats_unis_rss_feed_url,
                max_items=int(boursier_etats_unis_max_articles_total),
                mode="today" if boursier_etats_unis_mode == "Aujourd’hui" else "last_hours",
                hours_window=int(boursier_etats_unis_hours_window),
                ignore_time_filter=bool(boursier_etats_unis_rss_ignore_time_filter),
            )
            if boursier_etats_unis_rss_use_dom_fallback:
                dom_items = fetch_boursier_etats_unis_dom_items(
                    page_url="https://www.boursier.com/actualites/etats-unis",
                    max_items=int(boursier_etats_unis_max_articles_total),
                    mode="today" if boursier_etats_unis_mode == "Aujourd’hui" else "last_hours",
                    hours_window=int(boursier_etats_unis_hours_window),
                    use_firecrawl_fallback=bool(boursier_etats_unis_use_firecrawl),
                )
                st.session_state.boursier_etats_unis_rss_candidates = merge_article_items(
                    dom_items,
                    rss_items,
                    int(boursier_etats_unis_max_articles_total),
                )
            else:
                st.session_state.boursier_etats_unis_rss_candidates = rss_items
            boursier_etats_unis_job.status_log.append("🔎 URLs chargées")
            if not st.session_state.boursier_etats_unis_rss_candidates:
                st.warning("Aucune URL détectée. DOM vide ou bloqué ; active Firecrawl ou un RSS valide.")
            st.rerun()

    if boursier_etats_unis_clear:
        boursier_etats_unis_job.clear()
        st.session_state.boursier_etats_unis_rss_candidates = []
        st.session_state.boursier_etats_unis_show_json_state = False
        st.session_state.boursier_etats_unis_json_ready = False
        st.session_state.boursier_etats_unis_last_params = None
    ...
    boursier_etats_unis_status = boursier_etats_unis_job.get_status()
    boursier_etats_unis_state = boursier_etats_unis_status.get("state")
    boursier_etats_unis_total = boursier_etats_unis_status.get("total", 0)
    boursier_etats_unis_processed = boursier_etats_unis_status.get("processed", 0)
    boursier_etats_unis_skipped = boursier_etats_unis_status.get("skipped", 0)
    boursier_etats_unis_started_at = boursier_etats_unis_status.get("started_at")
    boursier_etats_unis_last_log = boursier_etats_unis_status.get("last_log", "")

    st.progress(boursier_etats_unis_processed / max(boursier_etats_unis_total, 1))
    status_parts = [
        f"{boursier_etats_unis_processed}/{boursier_etats_unis_total} traités",
        f"{boursier_etats_unis_skipped} ignorés",
    ]
    if boursier_etats_unis_state:
        status_parts.append(f"état: {boursier_etats_unis_state}")
    if boursier_etats_unis_started_at:
        elapsed = int(time.time() - boursier_etats_unis_started_at)
        status_parts.append(f"{elapsed}s")
    st.caption(" · ".join(status_parts))
    if boursier_etats_unis_last_log:
        st.caption(f"Statut : {boursier_etats_unis_last_log}")
    if boursier_etats_unis_status.get("errors"):
        st.markdown("**Erreurs :**")
        for err in boursier_etats_unis_status.get("errors")[-3:]:
            st.write(f"⚠️ {err}")
    if boursier_etats_unis_state in ("running", "paused"):
        st.info("Job en cours — rafraîchissement automatique activé.")
        time.sleep(2)
        st.rerun()

    if boursier_etats_unis_status.get("buffer_text"):
        st.divider()
        st.markdown("**Preview concaténée (buffer)**")
        boursier_etats_unis_edited_buffer = st.text_area(
            label="",
            value=boursier_etats_unis_status.get("buffer_text", ""),
            height=320,
            key="boursier_etats_unis_buffer_editor",
        )
        col_json, col_clear_buf = st.columns(2)
        with col_json:
            if st.button("✅ Dédoublonner + JSON", use_container_width=True, key="boursier_etats_unis_finalize"):
                boursier_etats_unis_job.set_buffer_text(boursier_etats_unis_edited_buffer)
                result = boursier_etats_unis_job.finalize_buffer()
                if result.get("status") == "success":
                    st.success(f"{len(result.get('items', []))} items générés")
                    st.session_state.boursier_etats_unis_json_ready = True
                    st.session_state.boursier_etats_unis_show_json_state = False
                    boursier_etats_unis_status = boursier_etats_unis_job.get_status()
                else:
                    st.error(result.get("message", "Erreur JSON"))
        with col_clear_buf:
            if st.button("🧹 Clear buffer", use_container_width=True, key="boursier_etats_unis_clear_buffer"):
                boursier_etats_unis_job.set_buffer_text("")
                st.rerun()

    if (
        st.session_state.boursier_etats_unis_json_ready
        and boursier_etats_unis_status.get("json_preview_text")
        and not st.session_state.boursier_etats_unis_show_json_state
    ):
        if st.button("🧾 Afficher preview JSON", use_container_width=True, key="boursier_etats_unis_show_json_btn"):
            st.session_state.boursier_etats_unis_show_json_state = True
            st.rerun()

    if boursier_etats_unis_status.get("json_preview_text") and st.session_state.boursier_etats_unis_show_json_state:
        st.markdown("**Preview JSON**")
        boursier_etats_unis_edited_json = st.text_area(
            label="",
            value=boursier_etats_unis_status.get("json_preview_text", ""),
            height=350,
            key="boursier_etats_unis_json_editor",
        )
        col_send, col_clear_json = st.columns(2)
        with col_send:
            if st.button("✅ Envoyer en DB", use_container_width=True, key="boursier_etats_unis_send_db"):
                result = boursier_etats_unis_job.send_to_db()
                if result.get("status") == "success":
                    st.success(f"{result.get('inserted', 0)} items insérés en base")
                else:
                    st.error(result.get("message", "Erreur DB"))
        with col_clear_json:
            if st.button("🧹 Clear JSON", use_container_width=True, key="boursier_etats_unis_clear_json"):
                boursier_etats_unis_job.json_preview_text = ""
                boursier_etats_unis_job.json_items = []
                st.session_state.boursier_etats_unis_show_json_state = False
                st.session_state.boursier_etats_unis_json_ready = False
